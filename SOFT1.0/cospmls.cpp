...
{
    float *prev, *prevprev;
    float *temp1, *temp2, *temp3, *temp4, *x_i, *eval_args;
    float *tableptr, *cosres, *cosworkspace;
    int i, j, k;
    ...
    for (i=0; i<bw-m-1; i++) {
	vec_mul((float)L2_cn(m,m+i),prevprev,temp1,bw);
	vec_pt_mul(prev, x_i, temp2, bw);
	vec_mul((float)L2_an(m,m+i), temp2, temp3, bw);
	vec_add(temp3, temp1, temp4, bw); /* temp4 now contains P(m,m+i+1) */
	/* compute cosine transform */
	kFCT(temp4, cosres, cosworkspace, bw, bw, 1);
	...
	/* now update Pi and P(i+1) */
	memcpy(prevprev, prev, sizeof(float) * bw);
	memcpy(prev, temp4, sizeof(float) * bw);
    }
}
...
float **Spharmonic_Pml_Table(int bw,
							  float *resultspace,
							  float *workspace)
{

	int i;
	float **spharmonic_pml_table;

	/* allocate an array of double pointers */
	spharmonic_pml_table = (float **) malloc(sizeof(float *) * bw);
...
float **Transpose_Spharmonic_Pml_Table(float **spharmonic_pml_table, 
					int bw,
					float *resultspace,
					float *workspace)
{
  ...
  int i;
  float **transpose_spharmonic_pml_table;

  /* allocate an array of double pointers */
  transpose_spharmonic_pml_table = (float **) malloc(sizeof(float *) * bw);
